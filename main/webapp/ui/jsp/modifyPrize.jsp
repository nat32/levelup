...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(
            function () {
                $.getJSON('<spring:url value="/levels"/>', {
                    ajax : 'true'
                }, function (data) {
                    var html = '<option value="0">Accessible au niveau:</option>';
                    var len = data.length;
                    for (var i = 1; i < len; i++) {
                        html += '<option value="' + data[i].id + '"><b>'
                            + data[i].number +  '</b> (points nécessaires - ' +data[i].points +')'+'</option>';
                    }
                    html += '</option>';
...
<h1 align="center">Modifier une récompense</h1>

    <a href="${pageContext.request.contextPath}/showPrizes/${user_id}" role="button" class="btn btn-secondary btn-lg btn-block">Voir la  liste récompenses</a>

<c:choose>
    <c:when test="${param.message==1}">
    <div class="alert alert-warning" role="alert">
        <p class="error"> Nom et  niveau de la récompense obligatoire</p>
    </div>
        <br />
    </c:when>
...
<p>Nom  : ${current_prize.name}</p>

<p>Accessible au niveau : ${current_prize.level_number} (points nécessaires) : ${current_prize.level_points}</p>


<h3>Modifier votre récompense</h3>

<form:form  method="POST" action="/levelup/modifiedPrize" modelAttribute="modified_prize">
    <form:hidden path="id" value="${current_prize.id}" />
    ...
    <div class="form-group">
        ...
        <form:select class="form-control" path="level_id" id="levels" ></form:select>
    </div>
